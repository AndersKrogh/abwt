...
#if FMITYPE==1
#define PRGNAME "makeabwt (using compact fmi)"
#elif FMITYPE==2
#define PRGNAME "makeabwt (using simple fmi)"
#else
#define PRGNAME "makeabwt (using funny fmi)"
#endif
...
static char *number2word(int n, int alen, char *w, int wlen, char *alphabet) {
  ...
  w[i]=0;
  if (alphabet) for (i=0; i<wlen; ++i) w[i]=alphabet[(int)w[i]];
  return w;
}
...
void *BucketSorter(void *x) {
  ...
  while( bs->bwtwrite < bs->nbuckets ) {
    ...
    if ( bs->sawrite >=0 && bs->sawrite < bs->nbuckets && bs->b[bs->sawrite]->status == DONEBWT ) {
      b=bs->b[bs->sawrite];
      b->status=PROCESS;
      pthread_mutex_unlock(&(bs->lock));
...
static int compare_Sequence(const void *x1, const void *x2) {
  Sequence *ss1, *ss2;
  ss1 = *(Sequence **)x1;
  ss2 = *(Sequence **)x2;
  return compare_strings( (void*)&(ss1->s), (void*)&(ss2->s) );
}
...
/*
  ...
  Assumes that ->sort_order holds the terminator sort order when function
  is called

  Initial Sequence points to first sequence and total

 */
static void SortSeqs(Sequence *ss, suffixArray *s) {
  int i, nseq = s->nseq;
  Sequence **ssarray, *cur;

  ssarray = (Sequence **)malloc(nseq*sizeof(Sequence*));
  ...
  qsort(ssarray,nseq,sizeof(Sequence*),compare_Sequence);
  ...
  for (i=0; i<nseq; ++i) {
    ...
    if (checkBit(cur->flag,seq_flag_comp)) {  // Reverse comp has negative length
      s->ids[i] = ((Sequence*)(cur->id))->id;
...
static int compare_Sequence_reverse(const void *x1, const void *x2) {
  Sequence *ss1, *ss2;
  char *s1, *s2;
  ...
  //  return compare_reverse(ss1->s+ss1->len, ss2->s+ss2->len);
...
void revSortSeqs(Sequence *ss) {
  ...
  cur = ss->next;
  for (i=0; i<nseq; ++i) { ssarray[i]=cur; cur=cur->next; }
  qsort(ssarray,nseq,sizeof(Sequence*),compare_Sequence_reverse);
...
/*
  ...
  Numbers are encoded with NEGATIVE characters
*/
void encodeOrder(Sequence *ss, int alen) {
  ...
  cur = ss->next;
  while (cur) {
    number2word(cur->sort_order, bigAlen, cur->s+cur->len, wl, alphabet);
    /*
    fprintf(stdout,"%s %d",cur->id, cur->sort_order);
    for (i=0; i<wl; ++i) fprintf(stdout,"%5d",*(cur->s+cur->len+i));
    fprintf(stdout,"\n");
    */
    cur=cur->next;
  }
...
void write_term(Sequence *ss, suffixArray *sa, FILE *fp) {
  Sequence *cur;
  int i;
  ...
  while (cur) {
    ...
    bwt[i] = *(cur->s+cur->len-1);
    // printf("i=%d l=%d bwt=%d %s\n",i,cur->len,bwt[i],cur->id);
    cur = cur->next;
  }
...
void reverse_sequences(Sequence *seq) {
  while (seq->next) {
    seq=seq->next;
    reverseSequence(seq);
  }
...
#include "makeabwt_vars.inc"
  ...
  if (infilename) fp = open_file_read(infilename, NULL, "makeabwt");
  else {
    if (!approx_seqlen) ERROR("makeabwt: You need to specify length (-l) in MB, when sequences are read on stdin",2);
    fp = stdin;
  }

  /* Check if output file name is given */
  if (!outfilename) {
    if (!infilename) ERROR("makeabwt: You need to specify name for output file if sequences are read on stdin",2);
  ...
  //astruct = bio_AlphabetStruct(Alphabet, caseSens, 0, wildcard, 0);
  // alphabet = astruct->a;
  // alen = astruct->len;

  /* Set word length */
  if (word_len) wlen = word_len;
  else {
    wlen = 2;
    if (astruct->len<=16) wlen = 3;
    if (astruct->len<=8)  wlen = 4;
  }
  if (padding<wlen) padding=wlen;

  /* Read sequences from fasta file */
  ss = readLongFasta(fp, approx_seqlen, astruct, revComp, 0, padding);
  if (infilename) fclose(fp);

  print_time("Sequences read");

  if (reverse) {
    reverse_sequences(ss);
    reverse=1;                   // Bit 1 is reverse BWT
    print_time("Sequences reversed");
  }
  if (revComp) reverse+=2;       // Bit 2 means the reverse complement is included


  fprintf(stderr,"SLEN %ld\nNSEQ %d\nALPH %s",ss->len,ss->sort_order,astruct->a);
  if (astruct->comp) fprintf(stderr," (%s)",astruct->comp);
  fprintf(stderr,"\n");

  /* ss->next points to the first seq. Other fields of ss contains other info */
#ifdef DEBUG2
  print_seq(ss->s, ss->s, 0, ss->len, 50, astruct->a, astruct->len, stderr);
#endif

  /* Open files */
  bwtfile = open_file_write(outfilename,"bwt","makeabwt");
  sa_file = open_file_write(outfilename,"sa","makeabwt");
  ...
  bwt = alloc_BWT(bwtlen,nseq,(uchar)reverse,astruct);
  write_BWT_header(bwt,bwtfile);
  // We do not want to free alphabet with the BWTbucket
  bwt->astruct=NULL;
  free_BWT(bwt);
  ...
  encodeOrder(ss,astruct->len);

  DEBUG1LINE(fprintf(stderr,"Order encoded\n"));

  /* Alloc bucket stack */
  BucketStack *wbs = initBucketStack(astruct->len,astruct->a,ss->len,ss->s,bwtfile, sa_file, sa_struct, wlen);
  ...
  /* Now read in the bwt and SA, construct FMI and write final index
     (This was previously done by mkfmi)
   */
  fprintf(stderr,"Reading BWT and suffix array again and construct FM index\n");
  construct_fmi_from_files(outfilename, 1, 1);
